...
- **Unit notes card in Unit Search** — `units.notes` is populated for Curve (12 units) but not yet displayed. Small v0.28 change.
- **General-purpose bulk import (sheet per table)** — replace the hand-run Curve import with a committee-usable upload: units, people, vehicles, pets, keys, breaches, agents, notes. Scoped 1 to 2 days.
