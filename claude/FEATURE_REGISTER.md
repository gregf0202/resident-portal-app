...
**Last updated: 22 August 2026** · Maintained at every stage of development.
...
## 2 · Built in the last four weeks *(late July → 22 August 2026)*

- **Maintenance Report v2** *(v0.26.0, 22 August, built; awaiting deploy)* — the report now opens with a one-page summary: five tiles (items, resolved, open, average days to resolve, **Approved this period** in dollars), an amber **"N items are waiting on your decision"** panel listing each item with its recommended quote, amount, the date it was sent to vote and days waiting, a **waiting-on breakdown** with bars, the oldest open item, and the category split. Detail follows, grouped by what each item is waiting on (committee decision · a quote · contractor booked · lot owner/access · ready to schedule · not set); resolved items collapse to a compact table with an average and a period total. Supporting changes: a per-item **Waiting on** control in the Triage card (`m.waitingOn` + `m.waitingSince`, existing JSONB; derived from the workflow trail when unset); **exactly one ACCEPTED quote per item** (accepting a workflow quote closes its siblings in `db.js`; the report also de-duplicates across the legacy register and the workflow, marking extras "superseded"); a **duplicate-title nudge** on the report form; demo seed gives every issue a distinct title and a waiting-on value. Exact build of the 4 August mock.

- **Free tools 19 and 21** *(22 August)* — see the Free Tools manifest.
...
- **Set up by** *(planned 22 Aug)* — the building record always says who established it and who owns it. A quiet one-line panel on the building record and committee dashboard: *"Set up by [name, role], [date]. This is the building's record. The owners corporation can export it in full at any time."* Provenance (established-by, date, funding party, novation date) rides existing JSONB on the building, no migration. Wording adapts: committee-funded reads "Set up by the Committee" and drops the ownership sentence; BM-funded keeps it. Changing the funding party is a committee action, never a BM one. Closes the gap between "the committee owns the record" and a BM-paid install, and is the visible proof behind the *Your Building, Your Records* proposition. Spec in `claude_POSITIONING_YOUR_BUILDING_YOUR_RECORDS.md`.
- **Export everything** *(planned 22 Aug)* — an always-visible committee-only control beside the Set up by line, producing the full record in portable form: documents, correspondence, maintenance history with quotes and decisions, contracts, registers, minutes. Export events are logged and visible (when, by whom), so the ownership claim has an audit trail. Doubles as the BM handover pack (BM build list item 5). Extends the existing Data export.
...
| 22 August | **Own the record, named** | *Your Building, Your Records* elevated to core proposition; Set up by + Export everything specified; Records Literacy one-pager built as free tool 21 |
| 22 August | **Delays go where they belong** | Maintenance Report v2 built (v0.26.0): summary first, waiting-on grouping, decision panel with dates sent, approved spend |
