...
- **Set up by** *(v0.27.0, 22 August, built; awaiting deploy)* — the building record now says who established it and who funds it, in one quiet line on the committee dashboard and a card in Settings. Stamped once at the final Getting Started gate (or on building creation); "Change who funds NaloHub" is committee-only and append-only, with a novation line ("Handed to the Committee on …"). BM sees the line, not the control. Residents never see it. Buildings that predate this get a "Record it now" backfill.
- **Export everything** *(v0.27.0, 22 August, built; awaiting deploy)* — the existing one-tap building export (every register, vote, contract, walk-through, audit trail, 7-day links to every stored file), retitled and now logged: each run is recorded on the building and the last five show as an **Export log** (who, when, how many sheets and files) under the Set up by card. Fixed on the way through: the export was pulling document and gallery file payloads into the workbook; it now reads the metadata views, so a building with a 20 MB minutes PDF no longer produces a 27 MB spreadsheet.
...
| 22 August | **The record says whose it is** | Set up by + Export everything built (v0.27.0); export log visible; file-payload leak in the export fixed |
