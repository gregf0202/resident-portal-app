...
import {
  ...
  Boxes, Bell, DollarSign, Smartphone,
...
import { audit, searchLegislation, loadDisputes, createDispute, appendDisputeEvent, setDisputeStatus, verifyDisputeChain, uploadAttachment, attachmentUrl,
  ...
  listWalkItems, seedWalkDefaults, listWalks, createWalk, listWalkResults, setWalkResult, completeWalk } from "./db.js";
...
const PLATFORM = { name: "Resident Portal", version: "0.12.0" }; // 0.12.0: committee suite — Voting (motions/majority/proxies), Maintenance Workflow (triage→quotes→vote→contractor), Contracts, Contractors, Walk-Through
...
export function BuildingApp() {
  ...
  const visible = NAV.filter((n) => n.show(user.role) && moduleOn(building, n.key) && (n.key !== "billing" || backend) && (!n.premium || !backend || building.premiumSuite === true) && (user.role !== "strata" || ["dashboard", "announcements", "help"].includes(n.key)));
...
const useAuthId = () => {
  ...
  useEffect(() => { supabase.auth.getSession().then(({ data }) => setUid(data && data.session ? data.session.user.id : null)); }, []);
  return uid;
};
...
function VotingView() {
  const { backend } = useApp();
  if (!backend) return <DemoOnly title="Voting" sub="Committee motions decided in-app, with a full audit trail" hint="Motions, majority voting, comments and proxies run against the live database in production." />;
  return <VotingLive />;
}
function VotingLive() {
...
function MaintWorkflowView() {
  const { backend } = useApp();
  if (!backend) return <DemoOnly title="Maintenance Workflow" sub="Triage → quotes → recommendation → vote → contractor confirmed" hint="The sub-committee works each issue end-to-end here in production: alerts, triage, quote collation, committee vote and contractor confirmation — one auditable trail." />;
  return <MaintWorkflowLive />;
}
function MaintWorkflowLive() {
  ...
  const toVote = async (q) => {
    try {
      await createMotion(buildingId, uid, { title: `Accept ${q.supplier_name} quote${q.amount ? ` $${q.amount}` : ""} — ${sel.title || sel.id}`, description: "Raised from the maintenance workflow.", context_type: "maintenance", context_id: sel.id, details: { quote_id: q.id } });
      await act("vote_opened", `Motion opened to accept ${q.supplier_name}.`, { quote_id: q.id });
      flash("Motion opened — BCC members alerted. Track it in Voting.");
    } catch (e) { flash(String(e.message || e)); }
  };
...
function ContractsView() {
  const { backend } = useApp();
  if (!backend) return <DemoOnly title="Contracts" sub="Every contract the building holds — term, expiry and value at a glance" hint="Party, purpose, dates, renewal terms and value, with expiry warnings — live in production." />;
...
function ContractsLive() {
  ...
  const save = async () => {
    ...
    try {
      await saveContract(buildingId, { ...f, term_months: f.term_months ? Number(f.term_months) : null, value_annual: f.value_annual ? Number(f.value_annual) : null, start_date: f.start_date || null, end_date: f.end_date || null });
      ...
      setAdding(false); reload(); flash("Contract recorded");
    } catch (e) { flash(String(e.message || e)); }
  };
    ...
    <div>
      ...
      <Wrap>
        ...
        {adding && (<Card style={{ padding: 18 }}><div className="grid sm:grid-cols-2 gap-3">
          ...
          <Field label="Category"><Input value={f.category} onChange={(e) => setF({ ...f, category: e.target.value })} placeholder="lift / cleaning / gardening / insurance" /></Field>
...
function ContractorsView() {
  const { backend } = useApp();
  if (!backend) return <DemoOnly title="Contractors" sub="Your approved and preferred trades" hint="Electrical, structural, cleaning, painting — with licence numbers, insurance expiry and ratings, live in production." />;
  return <ContractorsLive />;
}
    ...
    <div>
      ...
      <Wrap>
        ...
        {adding && (<Card style={{ padding: 18 }}><div className="grid sm:grid-cols-2 gap-3">
          ...
          <Field label="Trade"><Input value={f.trade} onChange={(e) => setF({ ...f, trade: e.target.value })} placeholder="electrical / structural / cleaning / painting" /></Field>
          <Field label="Contact"><Input value={f.contact_name} onChange={(e) => setF({ ...f, contact_name: e.target.value })} /></Field>
...
function WalkthroughView() {
  const { backend } = useApp();
  if (!backend) return <DemoOnly title="Walk-Through" sub="The monthly BM + BCC building inspection, on rails" hint="A 22-item standard checklist (fire safety, lifts, common areas, grounds…) that the building manager and committee tick off together — issues flow into maintenance." />;
  return <WalkthroughLive />;
}
function WalkthroughLive() {
  const { T, buildingId, user, flash } = useApp();
  ...
  useEffect(() => { reload(); }, [buildingId]);
  const start = async () => {
    try { const id = await createWalk(buildingId, attendees || user.name); setWalkId(id); setResults({}); setNotes({}); reload(); }
    catch (e) { flash(String(e.message || e)); }
  };
  const openWalk = async (w) => {
    ...
    const m = {}; const n = {};
    rs.forEach((r) => { m[r.item_id] = r.result; if (r.note) n[r.item_id] = r.note; });
    setResults(m); setNotes(n);
  };
  const mark = async (item, result) => {
    try { await setWalkResult(walkId, item.id, result, notes[item.id]); setResults({ ...results, [item.id]: result }); }
    catch (e) { flash(String(e.message || e)); }
  };
  const finish = async () => {
      ...
      <Wrap>
        ...
        {items.length > 0 && !walkId && (<>
          <Card style={{ padding: 18 }}>
            ...
            <Field label="Attendees"><Input value={attendees} onChange={(e) => setAttendees(e.target.value)} placeholder="e.g. B Manager (BM), B Chair (BCC)" /></Field>
            <Btn grad style={{ marginTop: 10 }} onClick={start}><ClipboardList size={15} /> Start walk-through</Btn>
          </Card>
          <SectionTitle>Past walks</SectionTitle>
          {walks.length === 0 && <Empty icon={ClipboardList} title="No walks recorded yet" />}
          {walks.map((w) => (<Card key={w.id} style={{ padding: 14 }}><div className="flex items-center gap-3"><div className="flex-1"><div className="font-semibold text-sm">{fmtDate(w.walk_date)}</div><div className="text-xs" style={{ color: T.textMuted }}>{w.attendees || ""}{w.summary ? ` · ${w.summary}` : ""}</div></div><Badge color={w.status === "completed" ? SEMANTIC.ok : SEMANTIC.warn}>{w.status.replace(/_/g, " ")}</Badge>{w.status === "in_progress" && <Btn kind="ghost" onClick={() => openWalk(w)}>Resume</Btn>}</div></Card>))}
        </>)}
        {walkId && (<>
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm flex-1" style={{ color: T.textMuted }}>{Object.keys(results).length}/{items.length} checked{current ? ` · ${current.attendees || ""}` : ""}</div>
          ...
          {areas.map((area) => (<Card key={area} style={{ padding: 16 }}>
            ...
            {items.filter((i) => i.area === area).map((i) => (
              <div key={i.id} className="py-2.5" style={{ borderBottom: `1px dashed ${T.border}` }}>
                ...
                {results[i.id] === "issue" && <Input placeholder="What's wrong? (also report it in Maintenance)" value={notes[i.id] || ""} onChange={(e) => setNotes({ ...notes, [i.id]: e.target.value })} onBlur={() => mark(i, "issue")} style={{ marginTop: 8 }} />}
              </div>))}
          </Card>))}
        </>)}
      </Wrap>
