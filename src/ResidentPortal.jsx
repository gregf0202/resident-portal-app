  ...
  return (
    <div className="flex">
      ...
      <main className="flex-1 min-w-0 md:ml-64 flex flex-col min-h-screen">
        ...
        <div className="flex-1" data-tour="main-content"><ViewRouter /></div>
        <Footer />
      </main>
    </div>
  );
                ...
                <div key={m.id} className="flex" style={{ justifyContent: out ? "flex-end" : "flex-start" }}>
                  <div className="max-w-[85%] rounded-2xl px-4 py-3" style={{ background: out ? hexToRgba(T.accent, T.mode === "dark" ? 0.16 : 0.10) : T.surfaceAlt, border: `1px solid ${out ? hexToRgba(T.accent, 0.35) : T.border}` }}>
                    ...
                    {m.deletedAt ? <div className="text-sm italic" style={{ color: T.textMuted }}>Message removed</div>
                      : <div className="text-sm whitespace-pre-wrap">{m.bodyText || (m.bodyHtml ? <span dangerouslySetInnerHTML={{ __html: m.bodyHtml }} /> : <span style={{ color: T.textMuted }}>(no body)</span>)}</div>}
                    {(m.attachments || []).map((a) => (
    ...
    <div>
      ...
      <Wrap>
        ...
        {items.length === 0 && (<Card style={{ padding: 18 }}>
          ...
          <Btn grad onClick={async () => { try { await seedWalkDefaults(buildingId); reload(); flash("Standard checklist loaded"); } catch (e) { flash(String(e.message || e)); } }}>Load standard checklist</Btn>
        </Card>)}
        {items.length > 0 && !walkId && (<>
          <Card style={{ padding: 18 }}>
            ...
            <div className="flex gap-2 mt-3.5"><Btn grad onClick={start}><ClipboardList size={15} /> Start walk-through</Btn><Btn kind="ghost" onClick={() => setEditing(!editing)}><Pencil size={14} /> {editing ? "Done editing" : "Edit checklist"}</Btn></div>
          </Card>
      ...
      <Wrap>
        <Card style={{ padding: 18 }}><SectionTitle right={<span style={{ color: T.textMuted }} className="text-[11px]">Word · minute-ready</span>}>Maintenance report for the BCC</SectionTitle>
          ...
          <div className="grid sm:grid-cols-3 gap-3 items-end">
            ...
            <Btn grad onClick={exportMaintReport} disabled={mr.busy}><Download size={15} /> {mr.busy ? "Building report…" : "Word report"}</Btn>
          </div>
        </Card>
...
function Directory() {
  ...
  return (
    <div>
      <Head title="Directory" sub={canManage ? "Neighbours listed — plus committee management" : "Neighbours who chose to be listed"} action={canManage && <HeaderAction onClick={() => { setAdding((v) => !v); setEditId(null); }}><Plus size={16} /> Add person</HeaderAction>} />
      <Wrap>
