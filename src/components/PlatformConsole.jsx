...
export default function PlatformConsole({ authUser, profileName, onOpen, onSignOut }) {
  ...
  const savePassword = async () => {
    if (newPw.length < 8) { setPwMsg("Use at least 8 characters."); return; }
      ...
      <AnimatedHeader>
        <div style={{ maxWidth: 880, margin: "0 auto", padding: "26px 18px 64px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { setShowPw((v) => !v); setPwMsg(""); }} style={{ background: "transparent", color: "rgba(255,255,255,0.85)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 10, padding: "6px 10px" }}>Set password</button>
        ...
        {showPw && (
          <Card style={{ padding: 18, marginBottom: 16 }}>
            <Field label="New password (min 8 characters)">
              <Input type="password" value={newPw} autoComplete="new-password" placeholder="Choose a password"
                onChange={(e) => setNewPw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && savePassword()} />
            </Field>
            <div style={{ color: T.textMuted, fontSize: 12, marginBottom: 10 }}>Sets a password on your admin account so you can skip the emailed link.</div>
            {pwMsg && <div style={{ color: "#f87171", fontSize: 13, marginBottom: 10 }}>{pwMsg}</div>}
            <Btn onClick={savePassword} disabled={pwBusy}>{pwBusy ? "Saving…" : "Save password"}</Btn>
          </Card>
        )}
        {!showPw && pwMsg && <Card style={{ padding: 12, marginBottom: 12, color: "#34d399" }}>{pwMsg}</Card>}
