-- 0036 Managing agents as an audience, and several audiences in one notice.
--
-- Managing agents were reachable nowhere in NaloHub: Curve carries an agent on
-- 12 tenanted lots (11 agencies) on the unit record, and the register also has a
-- Managing agent contact type. Both are used here, de-duplicated on email, so an
-- agency managing four lots is emailed once.
--
-- p_audiences: ["residents","agents","list:<uuid>","specific"] resolves each and
-- unions them, de-duplicated the same way, so a person in two groups is emailed
-- once. Single-audience calls are unchanged. NOTE: this replaces the four-argument
-- signature, which is dropped at the end, so the app and send-announcement v11
-- must ship together.
create or replace function public.broadcast_recipients(
  ...
  p_audiences jsonb default null)
returns jsonb
...
begin
  ...
  -- Several audiences in one notice: resolve each, then merge.
  if p_audiences is not null and jsonb_array_length(p_audiences) > 0 then
    for v_tok in select jsonb_array_elements_text(p_audiences) loop
      if v_tok like 'list:%' then
        v_one := public.broadcast_recipients(p_building, 'list', substring(v_tok from 6)::uuid, '[]'::jsonb, null);
      elsif v_tok = 'specific' then
        v_one := public.broadcast_recipients(p_building, 'specific', null, v_keys, null);
      else
        v_one := public.broadcast_recipients(p_building, v_tok, null, '[]'::jsonb, null);
      end if;
      v_all := v_all || coalesce(v_one->'people', '[]'::jsonb);
    end loop;
    select jsonb_build_object(
      'audience', 'multi', 'audiences', p_audiences, 'list_name', null,
      'people', coalesce(jsonb_agg(p order by p->>'unit', p->>'name'), '[]'::jsonb),
      'count', count(*), 'emailable', count(p->>'email'),
      'no_email', count(*) - count(p->>'email'),
      'units', count(distinct (p->>'unit'))) into v_out
    from (select distinct on (coalesce(lower(p->>'email'), p->>'key')) p
          from jsonb_array_elements(v_all) p
          order by coalesce(lower(p->>'email'), p->>'key')) d;
    return v_out;
  end if;
  ...
  -- Managing agents: from the unit's agent details and from Managing agent
  -- contacts on the register. One agency, one email, however many lots it holds.
  if v_aud = 'agents' then
    select jsonb_build_object(
      'audience', coalesce(p_audience, v_aud), 'list_name', v_list_name,
      'people', coalesce(jsonb_agg(jsonb_build_object(
          'key', key, 'name', full_name, 'email', email, 'unit', unit_number,
          'kind', 'agent', 'lives_here', false, 'membership_id', null,
          'level', null, 'pet', false) order by unit_number, full_name), '[]'::jsonb),
      'count', count(*), 'emailable', count(email),
      'no_email', count(*) - count(email), 'units', count(distinct unit_number))
      into v_out
    from (
      select distinct on (lower(email)) key, full_name, email, unit_number
      from (
        select 'ua:' || u.id as key,
          coalesce(nullif(trim(u.agent_contact), ''), nullif(trim(u.agent_business), ''), 'Managing agent') as full_name,
          nullif(trim(u.agent_email), '') as email, u.unit_number
        from units u where u.building_id = p_building and nullif(trim(u.agent_email), '') is not null
        union all
        select 'up:' || p.id, coalesce(nullif(trim(p.full_name), ''), 'Managing agent'),
          nullif(trim(p.email), ''), u.unit_number
        from unit_people p join units u on u.id = p.unit_id
        where u.building_id = p_building and p.is_current and p.person_type = 'property_manager'
          and nullif(trim(p.email), '') is not null
      ) a
      order by lower(email), key
    ) x;
    return v_out;
  end if;
...
revoke all on function public.broadcast_recipients(uuid, text, uuid, jsonb, jsonb) from public, anon;
grant execute on function public.broadcast_recipients(uuid, text, uuid, jsonb, jsonb) to authenticated, service_role;
drop function if exists public.broadcast_recipients(uuid, text, uuid, jsonb);
